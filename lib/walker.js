...
_tokenProto.before = function(newToken) {
    ...
    if (! newToken.range) {
        newToken.range = [target.range[0], target.range[0] + newToken.value.length];
    }
    ...
    if (! newToken.loc) {
        newToken.loc = {
            start : {
                ...
                line : newToken.type !== 'LineBreak'? target.loc.start.line : target.loc.start.line + 1,
                column : target.loc.start.column + newToken.value.length
            }
        };
    }
    ...
    if (target.prev) {
        ...
        target._ast.startToken = newToken;
    }
...
_tokenProto.after = function(newToken) {
    ...
    if (! newToken.range) {
        newToken.range = [target.range[1], target.range[1] + newToken.value.length];
    }

    if (! newToken.loc) {
        newToken.loc = {
            start : {
                line : target.loc.end.line,
                column : target.loc.end.column
            },
            end : {
                line : newToken.type !== 'LineBreak'? target.loc.end.line : target.loc.end.line + 1,
                column : target.loc.end.column + newToken.value.length
            }
        };
    }
...
_tokenProto.remove = function(){
    if (this.next) {
        ...
        this._ast.endToken = this.prev;
    }
    if (this.prev) {
        this.prev.next = this.next;
    } else {
        this._ast.startToken = this.next;
    }
    this.next = undefined;
    this.prev = undefined;
};
...
function updateRangeTillEnd(startToken, diffColumns, diffLines){
    ...
    if (! diffColumns && ! diffLines) return;
    do {
...
function instrumentTokens(ast, source){
    ...
    tokens.forEach(function(token, i){
        ...
        token._ast = ast; // used internally
        // for brevity
